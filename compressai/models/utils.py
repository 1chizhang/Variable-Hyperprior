...
class conv_con(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=2):
        ...
        embed_dim = self.default_embedding_dim
        self.embedding_layer = nn.Sequential(
            nn.GELU(),
            nn.Linear(embed_dim, 2*out_channels),
            nn.Unflatten(1, unflattened_size=(1, 1, 2*out_channels))
        )
        self.require_lmb = True
...
class deconv_con(nn.Module):
    default_embedding_dim = 256
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=2):
        super(deconv_con, self).__init__()
